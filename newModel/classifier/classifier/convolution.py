...
def buildModel(inputShape):
    ...
    model.add(keras.layers.Conv2D(64, (3, 3), activation='relu', input_shape=inputShape, padding='same'))
    # batch normalisation 3
    model.add(keras.layers.BatchNormalization())
    # dropout of 30%
    model.add(keras.layers.Dropout(0.5))
    ...
    # convolutional layer 3
    # 32 filters, 2x2 kernel, ReLU as activation function
    model.add(keras.layers.Conv2D(64, (2, 2), activation='relu', input_shape=inputShape, padding='same'))
    # pooling layer 3
    # Max pooling, 2x2 grid, 2x2 stride, 0 padding around all the edges
    model.add(keras.layers.MaxPool2D((2, 2), strides=(2, 2), padding='same'))
    # batch normalisation 3
...
if __name__ == "__main__":
    print("I am a classifier with convolution")
    ...
    # plot results
    predictions = model.predict(xTest, batch_size = 32, verbose = 1)
